...
{
    public class S_5_2_15_DepartmentMonthlyWorkingHoursReport : BaseServices, I_5_2_15_DepartmentMonthlyWorkingHoursReport
    {
        public S_5_2_15_DepartmentMonthlyWorkingHoursReport(DBContext dbContext) : base(dbContext)
        {
        }

        public async Task<OperationResult> GetData(DepartmentMonthlyWorkingHoursReportParam param, string userName)
        {
            var results = new DepartmentMonthlyWorkingHoursReportDto
            {
                param = param,
                print_By = userName,
            ...
            var departmentGroups = dataPeronals.GroupBy(x => new { x.Department }).Select(x => x.Key.Department).ToList();
            var dataExcel = new List<DataExcel>();
            foreach (var department in departmentGroups)
            {
                // Số lượng nhân viên theo phòng ban
                var employeesOfDepartment = dataPeronals.Where(x => x.Department == department).Select(x => new PersonalInfo
                {
                    Swipe_Card_Option = x.Swipe_Card_Option,
                    Employee_ID = x.Employee_ID,
                    Work_Shift_Type = x.Work_Shift_Type,
                    Onboard_Date = x.Onboard_Date
                }).ToList();
                // Tính tổng số giờ làm việc theo ngày theo phòng ban
                var normal_Working_Hours = await CalculatorNormal_Working_Hours(employeesOfDepartment, param.factory, firstDate.Value, lastDate.Value);
                var overtime_Hour = CalculatorOvertime_Hour(employeesOfDepartment, HAOM);
                var data = new DataExcel
                {
                    department = dataDepartments.FirstOrDefault(x => x.Department_Code == department)?.Department_Code ?? department,
                    department_Name = dataDepartments.FirstOrDefault(x => x.Department_Code == department)?.Department_Name,
                    total_Headcount = employeesOfDepartment.Count,
                    new_Recruits = employeesOfDepartment.Count(x => x.Onboard_Date >= firstDate && x.Onboard_Date <= lastDate),
                    normal_Working_Hours = normal_Working_Hours,
                    overtime_Hour = overtime_Hour,
                    total_Working_Hours = (normal_Working_Hours + overtime_Hour) ?? 0,
                    average_Total_Working_Hours_Per_Person = ((normal_Working_Hours + overtime_Hour) / employeesOfDepartment.Count) ?? 0
                };
                dataExcel.Add(data);
            }
        ...
        private static decimal? CalculatorOvertime_Hour(List<PersonalInfo> personalByDeps, HashSet<HRMS_Att_Overtime_Maintain> HAOM)
        {
            decimal? overtime_Hour = 0;
            personalByDeps.ForEach(item =>
            {
                var data = HAOM.Where(x => x.Employee_ID == item.Employee_ID)
        ...
        private async Task<decimal> CalculatorNormal_Working_Hours(List<PersonalInfo> personalByDeps, string factory, DateTime firstDateOfMonth, DateTime lastDateOfMonth)
        {
            // Tổng số giờ làm việc theo giờ
            decimal normal_Working_Hours_Result = 0;

            // lọc theo danh sách nhân viên theo phòng ban
            foreach (var employee in personalByDeps)
            {
                decimal? normal_Working_Hours;
                // Thường hợp không cần bấm thẻ
                ...
                if (normal_Working_Hours != null)
                    normal_Working_Hours_Result += normal_Working_Hours.Value;
            };
            return normal_Working_Hours_Result;
        }
        ...
        public async Task<OperationResult> Excel(DepartmentMonthlyWorkingHoursReportParam param, string userName)
        {
            var data = await GetData(param, userName);
            var export = data.Data as DepartmentMonthlyWorkingHoursReportDto;
            ...
            List<Cell> dataCells = new()
            {
                ...
                new Cell("B4", export.print_By),
                new Cell("D4", export.print_Date)
            };
            ExcelResult excelResult = ExcelUtility.DownloadExcel(
                tables, dataCells, 
                "Resources\\Template\\AttendanceMaintenance\\5_2_15_DepartmentMonthlyWorkingHoursReport\\Download.xlsx"
            );
