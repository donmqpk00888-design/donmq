...
export class S_7_2_9_SalarySummaryReportExitedEmployeeService {
  get language(): string { return localStorage.getItem(LocalStorageConstants.LANG) }
  apiUrl: string = environment.apiUrl + "C_7_2_9_SalarySummaryReportExitedEmployee/"

  initData: SalarySummaryReportExitedEmployeeSource = <SalarySummaryReportExitedEmployeeSource>{
  ...
  programSource = signal<SalarySummaryReportExitedEmployeeSource>(structuredClone(this.initData));
  programSource$ = toObservable(this.programSource);
  setSource = (source: SalarySummaryReportExitedEmployeeSource) => this.programSource.set(source);
  clearParams = () => {
  ...
  getTotalRows(param: SalarySummaryReportExitedEmployeeParam) {
    param.language = this.language
    let params = new HttpParams().appendAll({ ...param });
    return this.http.get<OperationResult>(this.apiUrl + 'GetTotalRows', { params })
  }

  download(param: SalarySummaryReportExitedEmployeeParam) {
    param.language = this.language
    let params = new HttpParams().appendAll({ ...param });
